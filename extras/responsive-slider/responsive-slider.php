		...
		$slider = '<div class="responsive-slider flexslider">';
			...
			while ( $slides->have_posts() ) : $slides->the_post();
				...
				$slider .= '<li>';
					...
					$slider .= '<section id="slide-' . get_the_ID() . '" class="row">';
						global $post;
						$slider .= '<div class="col-sm-6">';
						...
						$slider .= '<div class="col-sm-6">';
							$slider .= '<header>';
								$slider .= '<h1><a href="' . get_post_meta( $post->ID, "_slide_link_url", true ) . '" title="' . the_title_attribute ( array( 'echo' => 0 ) ) . '" >' . get_the_title() . '</a></h1>';
								$slider .= '<h2><a href="' . get_post_meta( $post->ID, "_slide_link_url", true ) . '" title="' . the_title_attribute ( array( 'echo' => 0 ) ) . '" >' . get_post_meta( $post->ID, "_slide_subtitle", true ) . '</a></h2>';
							$slider .= '</header>';
							$slider .= '<article>';
								$slider .= '<p>' . get_post_meta( $post->ID, "_slide_excerpt", true ) . '</p>';
							$slider .= '</article>';
						$slider .= '</div>';
					$slider .= '</section><!-- #slide-x -->';
...
function responsive_slider_create_slide_metaboxes() {
    add_meta_box( 'responsive_slider_metabox_2', __( 'Subtitle', 'responsive-slider' ), 'responsive_slider_metabox_2', 'slides', 'normal', 'default' );
    ...
    add_meta_box( 'responsive_slider_metabox_3', __( 'Excerpt', 'responsive-slider' ), 'responsive_slider_metabox_3', 'slides', 'normal', 'default' );
}
 ...
 * @since 0.1
 */             
function responsive_slider_metabox_2() {
	...
	<p><input type="text" style="width: 90%;" name="slide_subtitle" value="<?php echo esc_attr( $slide_subtitle ); ?>" /></p>
	<span class="description"><?php echo _e( 'Subtitle.', 'responsive-slider' ); ?></span>
 ...
 * @since 0.1
 */             
function responsive_slider_metabox_3() {
	
	global $post;	
             
	/* Retrieve the metadata values if they already exist. */
	$slide_excerpt = get_post_meta( $post->ID, '_slide_excerpt', true ); ?>
	
	<p><textarea style="width: 90%;" name="slide_excerpt"  ><?php echo esc_attr( $slide_excerpt ); ?></textarea></p>
	<span class="description"><?php echo _e( 'Excerpt.', 'responsive-slider' ); ?></span>
...
function responsive_slider_save_meta( $post_id, $post ) {
	...
	if ( isset( $_POST['slide_subtitle'] ) ) {
		update_post_meta( $post_id, '_slide_subtitle', strip_tags( $_POST['slide_subtitle'] ) );
	}
	if ( isset( $_POST['slide_excerpt'] ) ) {
		update_post_meta( $post_id, '_slide_excerpt', strip_tags( $_POST['slide_excerpt'] ) );
	}
}
